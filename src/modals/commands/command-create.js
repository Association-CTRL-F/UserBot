...
export default {
	...
	interaction: async (modal, client) => {
		...
		const active = modal.fields.getTextInputValue('active-command-create').trim()
		const contenu = modal.fields.getTextInputValue('content-command-create').trim()
		const buttonRaw = modal.fields.getTextInputValue('button-command-create').trim()
		...
		// Vérification si un alias existe déjà comme nom ou alias d'une autre commande
		if (aliasesList.length) {
			try {
				const sql = 'SELECT name, aliases FROM commands'
		...
		// On prépare éventuellement un bouton de redirection
		let button = null
		...
		const preview = `La commande **${nom}** a bien été créée et est ${statusText} 👌${
			aliases ? `\n\n__Alias :__\n\`\`\`${aliases}\`\`\`` : ''
